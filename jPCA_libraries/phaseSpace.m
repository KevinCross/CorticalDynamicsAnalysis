...
for pindex = 1:length(planes2plot)
    ...
    if useLabel
        ...
        temp_str=strcat(num2str(round(100*Summary.varCaptEachPlane(plane))),'% of var captured, freq:',num2str(Summary.MSkew_eigenvals(plane*2-1)/(2*pi)*100));
        titleText2=temp_str;
%         titleText2 = sprintf('%d%% of var captured', round(100*Summary.varCaptEachPlane(plane)));
        text(0,0.99*double(axLim(4)),titleText, 'horizo', 'center');
        text(0,0.88*double(axLim(4)),titleText2, 'horizo', 'center', 'fontSize', 8.5);
    end
