...
% uncomment out the task of interest
...
for net_type=13:2
    for lay_type=1:2
        ...
        curr_dir=strcat('data_network_update_15_07_2021/',task,'/',net_names{net_type},'/',lay_names{lay_type});
        
        %run jPCA analysis on data
        [Projection_Data,Summary_Data]=calculate_jPCA(curr_dir,offset,times);
        assert(false)
...
for net_type=1:2
    ...
    Neural_dir=strcat('data_network_update_15_07_2021/',task,'/',net_names{net_type},'/',lay_names{2});
    Sens_dir=strcat('data_network_update_15_07_2021/',task,'/',net_names{net_type},'/',lay_names{4});%note the kinematic signals are pulled from the sensory feedback
    %run jPCA analysis on the data
    bstrap_kine_and_muscle_signals(Neural_dir,Sens_dir,offset,task,times,net_names{net_type});
...
function []=bstrap_kine_and_muscle_signals(Neural_dir,Sens_dir,offset,task,times,net_name)
    niters=1000;
    [Data_Neural]=get_data_from_directory(Neural_dir,offset,times);
    [Data_Sens]=get_data_from_directory(Sens_dir,offset,times);
    ...
    Data_Mus=struct;
    for c=1:length(Data_Sens)
        Data_Mus(c).A=Data_Sens(c).A(:,end-5:end);
        Data_Mus(c).times=Data_Sens(c).times;
    end
        
    %perform jPCA and bootstrap analysis
    [Proj_Musc] = bstrap_neural_signals(Data_Mus,Data_Neural,times,6,niters,[1]); 
    [Proj_Kine] = bstrap_neural_signals(Data_Kin,Data_Neural,times,4,niters,[1]);
...
function [Projection,Summary_data] = bstrap_neural_signals(data,M1_data,times,ndim,niters,port_seq)
...
%     end
    [Projection,Summary_data]=jPCA_v2(data,times,jPCA_params);
    ...
    for it=1:niters
        ...
        for j=1:8
            ...
            temp(1,j).times=times;
        end
        [~,Summary]=jPCA_v2(temp,times,jPCA_params);
...
function [Projection_Data,Summary_Data]=calculate_jPCA(task,offset,times)
...
%     times=0:10:300;
    ...
    for i=3:size(files,1)
        ...
        for tp=1:size(tmp.(fname{1}),2)
            Data_struct(count).A=squeeze(tmp.(fname{1})(offset:offset+length(times)-1,tp,:));
    ...
    for j=1:length(Data_struct)
        data_tensor(j,:,:)=bigA(1+(j-1)*length(times):j*length(times),:);
    end
...
function [Data_struct]=get_data_from_directory(dir_name,offset,times)
    curr_dir=pwd;
    cd(dir_name)
    files=dir;
    Data_struct=struct;
